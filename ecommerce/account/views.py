...
def add_to_wishlist(request, id):
    ...
    if product.users_wishlist.filter(id=request.user.id).exists():
        ...
        messages.success(request, product.title + ' удален из вашего списка желаний')
    else:
        product.users_wishlist.add(request.user)
        messages.success(request, product.title + ' добавлен в ваш список желаний')
...
def set_default(request, id):
    Address.objects.filter(customer=request.user, pk=id, default=True).update(default=False)
    Address.objects.filter(customer=request.user, pk=id).update(default=True)
